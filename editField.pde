...
void setCan() {
  if (!lineDraw && mouseY>fieldY && mouseY<fieldY+800 && mouseX>fieldX && mouseX<fieldX+1600 && mouseButton==LEFT) {
    if (!canGrub) {
      for (byte i=0; i<3; i++)
...
void setRobot(){
  if (!lineDraw && mouseY>fieldY && mouseY<fieldY+800 && mouseX>fieldX && mouseX<fieldX+1600 && mouseButton==RIGHT){
